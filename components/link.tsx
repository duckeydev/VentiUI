...
export const linkVariants = cva(
  "inline-flex items-center gap-1 font-medium transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-sm relative group",
  {
    variants: {
      variant: {
        modern: "text-primary hover:text-primary/80 underline underline-offset-4 decoration-primary/30 hover:decoration-primary",
        minimal: "text-muted-foreground hover:text-foreground underline underline-offset-4 decoration-muted-foreground/20 hover:decoration-foreground",
        glass: "text-foreground font-semibold border-b border-white/20 dark:border-white/10 hover:border-primary/60 transition-colors pb-[1px]",
        macos: "text-foreground hover:text-primary no-underline",
    ...
    return (
      ...
      >
        <NextLink
          ...
          aria-label={isExternal ? `${children} (opens in a new tab)` : undefined}
          {...securityProps}
          {...props}
        >
          {children}
          {showIcon && isExternal && (
            <IconArrowUpRight
              className="inline-block w-3 h-3 text-muted-foreground/70 shrink-0 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
              aria-hidden="true"
            />
          )}
        </NextLink>
          ...
          {richPreview && isHovered && (
            ...
            >
              <span className="flex items-center gap-1.5 font-mono text-[11px] font-medium tracking-tight text-foreground/90 w-full overflow-hidden">
                <IconLink className="w-3 h-3 text-primary shrink-0" aria-hidden="true" />
              ...
              {info && (
                <span className="text-[10px] text-muted-foreground border-t border-border/40 mt-1 pt-1 block leading-normal">
                  <span className="font-semibold text-foreground/50">Redirects to:</span>{" "}
                  {info}
                </span>
              )}
            </motion.span>
