...
const deviceVariants = cva(
  "relative mx-auto border-neutral-900 dark:border-neutral-800 border-[12px] bg-neutral-950 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.35)] dark:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.65)] select-none shrink-0",
  {
    variants: {
      type: {
        phone: "w-[290px] h-[580px] rounded-[2.75rem]",
        tablet: "w-[580px] h-[390px] rounded-[2.25rem]",
      },
    },
    defaultVariants: {
      type: "phone",
...
export interface DeviceMockupProps
  extends React.PropsWithChildren<CleanHTMLAttributes>,
    VariantProps<typeof deviceVariants> {
  animate?: boolean;
  ...
  ({ type = "phone", children, className = "", animate = true, transition, ...props }, ref) => {
    ...
    return (
      <motion.div
        ...
        className={cn(
          deviceVariants({ type }),
          isPhone && "after:absolute after:bottom-2 after:left-1/2 after:-translate-x-1/2 after:w-28 after:h-1 after:bg-neutral-800/40 dark:after:bg-neutral-700/40 after:rounded-full",
          className
        )}
        {...props}
      >
        <div
          className={cn(
            "absolute left-1/2 -translate-x-1/2 bg-neutral-900 dark:bg-neutral-800 z-30 transition-colors",
            isPhone
        ...
        <div
          className={cn(
            "relative w-full h-full overflow-hidden bg-background border border-neutral-950/60 transition-colors",
