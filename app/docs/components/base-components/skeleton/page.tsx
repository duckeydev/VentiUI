...
const componentMeta = {
  title: "Skeleton",
  description: "A placeholder utility component used to mimic upcoming content layouts. Minimizes visual layout shifts during data-fetching states.",
  version: "v1.1.0",
  sourceUrl: "https://github.com/venti-ui/venti/blob/main/packages/skeleton.tsx",
};
...
const examples = [
  {
    ...
    description: "Combine multiple variants (circular, text, rectangular) to represent high-fidelity upcoming page blocks such as a dashboard card or user profile header.",
  ...
  },
  {
    ...
    title: "Base Variants",
    description: "The core geometrical options built directly into the tracking engine variant configurations.",
    ...
    render: () => (
      <div className="flex flex-col gap-4 w-full max-w-sm p-4 border border-border/40 rounded-xl bg-card/40 text-left">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-mono font-bold text-muted-foreground">circular</span>
        </div>
        <div className="space-y-1">
          <span className="text-xs font-mono font-bold text-muted-foreground">rectangular</span>
          <Skeleton variant="rectangular" className="h-16 w-full" />
        </div>
        <div className="space-y-1">
          <span className="text-xs font-mono font-bold text-muted-foreground">text</span>
          <Skeleton variant="text" />
        </div>
      </div>
    ),
...
const apiProperties = [
  { name: "variant", type: "'circular' | 'rectangular' | 'text'", default: "'rectangular'", description: "Dictates the basic architectural geometry configuration and applied border-radiuses." },
  { name: "className", type: "string", default: "''", description: "Bespoke classes used to apply strict sizing declarations (e.g., width, height) onto placeholders." },
];
...
export default function SkeletonDocsPage() {
  ...
  const [activeTabs, setActiveTabs] = useState<Record<string, "preview" | "code">>({
    "profile-card-loading": "preview",
    "standalone-variants": "preview",
  });
          ...
          {examples.map((example) => {
            ...
            return (
              <section key={example.id} id={example.id} className="space-y-3 scroll-mt-20">
                <div className="flex items-center gap-2">
                  <IconGridPattern className="h-4 w-4 text-primary animate-[pulse_3s_ease-in-out_infinite]" />
          ...
          <div className="flex items-center gap-2 border-b border-border/40 pb-2">
            ...
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">
                Properties parameters and layout types accepted by the Skeleton placeholder component.
              </p>
            </div>
          </div>
