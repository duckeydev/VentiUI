...
const componentMeta = {
  title: "Better Images™",
  description: "An enhanced multimedia platform wrapper extending Next.js asset management with structural aspect overlays, loading states, and automated error containment.",
  version: "v1.0.0",
...
const examples = [
  {
    ...
    description: "Declaratively enforce exact track dimensions using standardized token locks like standard wide vectors or perfect squares.",
...
export function Gallery() {
  ...
  },
  {
    ...
    description: "Gracefully isolates missing sources or runtime parsing crashes to block broken icon links from disrupting UI layers.",
    code: `import { Image } from "@/components/image";
...
const imageProperties = [
  { name: "src", type: "string | StaticImport", default: "required", description: "Source link pointing cleanly toward internal paths or verified cross-domain remote CDNs." },
  { name: "alt", type: "string", default: "required", description: "Accessibility alternative title string parsing screen readings directly for compliance vectors." },
  { name: "ratio", type: "'auto' | 'square' | 'video' | 'portrait' | 'landscape'", default: "'auto'", description: "Locks responsive container dimensions to specific geometric aspect-ratios." },
  { name: "roundness", type: "'none' | 'sm' | 'md' | 'lg' | 'full'", default: "'md'", description: "Applies unified design token border-radius structures smoothly onto outer framing borders." },
  { name: "fallback", type: "React.ReactNode", default: "calculated", description: "Custom UI node view to display if an asset fails to fetch or parse correctly." },
];

export default function ImageDocsPage() {
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const [activeTabs, setActiveTabs] = useState<Record<string, "preview" | "code">>({
    "standard-aspects": "preview",
  ...
  return (
    <DocsPageFrame
      ...
      rightBar={<DocsOutline title="On this page" items={[...examples.map(e => ({ label: e.title, href: `#${e.id}` })), { label: "Image API Reference", href: "#props-api" }]} />}
    >
      <main className="py-10 lg:col-span-7 space-y-12 lg:max-w-3xl">
        {/* Core Header Section Container */}
        <div className="space-y-3 border-b border-border pb-6">
          <DocsBreadcrumbs
            items={[
              { label: "Docs", href: "/docs" },
              { label: "Media Primitives", href: "/docs/components#media" },
              { label: "Image Component", href: "/docs/components/media/image" },
            ]}
          />
        ...
        {/* Configuration Properties Reference Matrix API Area */}
        <section id="props-api" className="space-y-4 scroll-mt-20">
          <div className="flex items-center gap-2 border-b border-border/40 pb-2">
            ...
            <div className="space-y-0.5">
              <h2 className="text-lg font-bold tracking-tight text-foreground">Image API Reference</h2>
              <p className="text-xs text-muted-foreground">
                Properties parameters and token definitions accepted by the extended media element wrapper.
              </p>
            </div>
          </div>
        ...
        <footer className="border-t border-border/30 pt-4 text-center text-xs text-muted-foreground/40">
          © 2026 Venti UI Labs. Automated asset compilation tracks.
        </footer>
      </main>
    </DocsPageFrame>
  );
