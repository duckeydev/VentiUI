...
export function DocsSplitLayout({
  ...
  return (
    <div
      className={`mx-auto ${maxWidthClassName} px-4 sm:px-6 lg:px-8 lg:grid lg:grid-cols-12 lg:gap-8`}
    >
      {leftSidebar}
      {children}
      {rightSidebar}
      ...
      {items.map((item, index) => {
        ...
        return (
          <span key={item.label} className="flex items-center gap-1.5">
            {index > 0 ? (
              <IconChevronRight className="h-3 w-3 stroke-3 text-muted-foreground/40" />
            ) : null}
            {isLast ? (
  ...
  return (
    <aside className="hidden py-10 text-xs lg:col-span-2 lg:block lg:h-[calc(100vh-3.5rem)] lg:sticky lg:top-14 lg:overflow-y-auto lg:pl-6 lg:pr-2 lg:border-l lg:border-border/40">
      <div className="space-y-4">
        <h5 className="text-[11px] font-bold uppercase tracking-widest text-foreground">
          {title}
        </h5>
          ...
          <ul className="relative z-10 space-y-3 pl-4 font-medium text-muted-foreground">
            {items.map((item) => (
              <li key={item.href} className="group relative">
                <a
                  href={item.href}
                  className="block truncate transition-all duration-150 hover:translate-x-0.5 hover:text-foreground"
                >
                  {item.label}
                </a>
              </li>
...
export function DocsPanel({ children, className }: DocsPanelProps) {
  return (
    <div
      className={`rounded-4xl border border-white/8 bg-[#11151d] ${className ?? ""}`.trim()}
    >
      {children}
    </div>
  );
